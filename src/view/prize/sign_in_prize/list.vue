  ...
  <div>
    ...
    <Modal v-model="modalSetting.show" width="700" :styles="{top: '30px'}" @on-visible-change="doCancel">
      <p slot="header" style="color:#2d8cf0;">
        ...
        <span>{{formItem.sign_in_prize_id ? '编辑' : '新增'}}</span>
      </p>
      <Form ref="myForm" :rules="ruleValidate" :model="formItem" :label-width="100">
        ...
        <FormItem label="第几天" prop="day">
          <Input v-model="formItem.day" placeholder="第几天"/>
        </FormItem>
      </Form>
      <div slot="footer">
...
<script>
  import {getDataList, coruData} from '@/api/sign_in_prize_list'
  ...
  const deleteButton = (vm, h, currentRow, index) => {
    return h('Poptip', {
      ...
      on: {
        'on-ok': () => {
          coruData({sign_in_prize_id: currentRow.sign_in_prize_id, is_delete: 1}).then(response => {
  ...
  export default {
    ...
    data() {
      return {
        ...
        columnsList: [{title: '资源类型', key: 'resources_type', align: 'center'}, {
          ...
          key: 'name',
          align: 'center'
        }, {title: '数量', key: 'number', align: 'center'}, {title: '第几天', key: 'day', align: 'center'}, {
          title: '创建日期',
          key: 'gmt_create',
          align: 'center'
        ...
        // 表单属性
        formItem: {sign_in_prize_id: '', resources_type: '', name: '', number: '', day: ''},
        // modal属性
        modalSetting: {
          show: false,
          loading: false,
    ...
    methods: {
      ...
      // 新增
      alertAdd() {
        this.formItem.sign_in_prize_id = 0

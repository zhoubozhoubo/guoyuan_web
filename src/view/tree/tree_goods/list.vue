  ...
  <div>
    ...
    <Modal v-model="modalSetting.show" width="700" :styles="{top: '30px'}" @on-visible-change="doCancel">
      <p slot="header" style="color:#2d8cf0;">
        ...
        <span>{{formItem.tree_goods_id ? '编辑' : '新增'}}</span>
      </p>
      <Form ref="myForm" :rules="ruleValidate" :model="formItem" :label-width="100">
        ...
        <FormItem label="钻石数" prop="diamond">
          <Input v-model="formItem.diamond" placeholder="钻石数"/>
        </FormItem>
      </Form>
      <div slot="footer">
...
<script>
  import {getDataList, coruData} from '@/api/tree_goods_list'
  ...
  const editButton = (vm, h, currentRow, index) => {
    return h('Button', {
      ...
      on: {
        'click': () => {
          vm.formItem.tree_goods_id = currentRow.tree_goods_id;
  ...
  const deleteButton = (vm, h, currentRow, index) => {
    return h('Poptip', {
      ...
      on: {
        'on-ok': () => {
          coruData({tree_goods_id: currentRow.tree_goods_id, is_delete: 1}).then(response => {
    ...
    data() {
      return {
        ...
        columnsList: [{title: '资源类型', key: 'resources_type', align: 'center'}, {
          ...
          key: 'name',
          align: 'center'
        }, {title: '数量', key: 'number', align: 'center'}, {title: '钻石数', key: 'diamond', align: 'center'}, {
          title: '状态',
          key: 'status',
          align: 'center'
        ...
        // 搜索配置
        searchConf: {resources_type: '', name: '', status: '', gmt_create: ''},
        // 表单属性
        formItem: {tree_goods_id: '', resources_type: '', name: '', number: '', diamond: ''},
        // modal属性
        modalSetting: {
          show: false,
          loading: false,
        ...
        ruleValidate: {
          ...
          diamond: [{required: true, message: "请输入钻石数", trigger: "blur"}],
        }
      }
    },
    created() {
              ...
              return h('i-switch', {
                ...
                on: {
                  'on-change': function (status) {
                    coruData({tree_goods_id: currentRowData.tree_goods_id, status: status}).then(response => {
                      vm.$Message.success(response.data.msg)
                    }).catch(() => {
                    })
                  }

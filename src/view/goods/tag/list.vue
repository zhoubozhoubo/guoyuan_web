    ...
    <div>
        ...
        <Modal v-model="modalSetting.show" width="700" :styles="{top: '30px'}" @on-visible-change="doCancel">
            <p slot="header" style="color:#2d8cf0;">
                ...
                <span>{{formItem.tag_id ? '编辑' : '新增'}}</span>
            </p>
            <Form ref="myForm" :rules="ruleValidate" :model="formItem" :label-width="100">
                                        ...
                                        <div class="demo-upload-list" v-if="formItem.icon">
                    ...
                    <input v-if="formItem.icon" v-model="formItem.icon" type="hidden" name="image">
                    <Upload type="drag"
                            v-if="!formItem.icon"
                            :action="uploadUrl"
...
<script>
    ...
    import { getToken } from '@/libs/util'
        import {getDataList,coruData} from '@/api/tag_list'
    ...
    const editButton = (vm, h, currentRow, index) => {
        return h('Button', {
            ...
            on: {
                'click': () => {
                                        vm.formItem.tag_id = currentRow.tag_id;
                                        vm.formItem.name = currentRow.name;
                                        vm.formItem.icon = currentRow.icon;
    ...
    const deleteButton = (vm, h, currentRow, index) => {
        return h('Poptip', {
            ...
            on: {
                'on-ok': () => {
                    coruData({ tag_id: currentRow.tag_id, is_delete: 1 }).then(response => {
        ...
        data() {
            return {
                ...
                uploadHeader: { 'apiAuth': getToken() },
                                // 初始化表格列
                columnsList:[{title:'名称',key:'name',align:'center'},{title:'图标',key:'icon',align:'center'},{title:'创建日期',key:'gmt_create',align:'center'},{title:'操作',key:'handle',align:'center',handle:['edit','delete']}],
                // 表格数据
                tableData: [],
                // 表格显示分页属性
                tableShow: {
        ...
        methods: {
                        ...
                        item.render = (h, param) => {
                            ...
                            if (currentRowData.icon) {
                                return h('img', {
                                    ...
                                    attrs: {
                                        src: currentRowData.icon,
                                shape: 'square',
                                    size: 'large'
                            },
                                on: {
                                    click: (e) => {
                                        vm.modalSeeingImg.img = currentRowData.icon;
            ...
            handleImgSuccess(response) {
                if (response.code === 1) {
                    ...
                    this.formItem.icon = response.data.fileUrl;
                } else {
